                   ...
                   description_event->server_version)
{
  ...
  server_uuid= strndup(pos, server_uuid_len);
  pos += server_uuid_len;

  encoded_snapshot_version=
      reinterpret_cast<const unsigned char *>(strndup(pos,
                                                           encoded_snapshot_version_length));
...
err:
  ...
  if (encoded_snapshot_version)
    free((void*)encoded_snapshot_version);
  encoded_snapshot_version= NULL;
  clear_set(&write_set);
  clear_set(&read_set);
  return;
                                                     ...
                                                     uint32_t remaining)
{
  ...
  for (uint32_t i= 0; i < set_len; i++)
  {
    ...
    if (remaining < len)
      return(NULL);
    const char *hash= strndup(pos, len);
    if (hash == NULL)
      return(NULL);
    pos += len;
    remaining-= len;
